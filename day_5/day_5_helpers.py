...
def insert_friendship_if_not_exists(friendship: Friendship, students_collection: Collection, friendships_collection: Collection) -> None:
    ...
    """
    if friendships_collection.find_one({"person_low_id": friendship.person_low_id, "person_high_id": friendship.person_high_id}) is None:
        friendships_collection.insert_one(friendship.model_dump())
        return True
    else:
        print(f"Friendship {friendship.person_low_id} {friendship.person_high_id} already exists!")
        return False
...
class FriendshipFeatures(BaseModel):
    # Similarities
    section_same: int | None = None  # 1 if the same, 0 if different
    favorite_subject_same: int | None = None  # 1 if the same, 0 if different

    # Jaccard similarities
    sport_jaccard: float | None = None  # Jaccard similarity between sports
    music_genre_jaccard: float | None = None  # Jaccard similarity between music genres
    hobby_jaccard: float | None = None  # Jaccard similarity between hobbies

    # Differences
    social_style_diff: int | None = None # abs difference in social_style
    academic_interest_diff: int | None = None # abs difference in academic_interest
    sibling_count_diff: int | None = None  # abs difference in number of siblings
    screen_time_diff: float | None = None  # abs difference in average daily screen time in minutes
    phone_pickups_diff: float | None = None  # abs difference in average daily phone pickups


class Friendship(BaseModel):
    # Friendship Type
    is_positive_friendship: bool | None = None  # 1 if positive, 0 if strangers/not friends
    # Linkage
    person_low_id: str | None = None  # keep sorting of ids so one one direction link can exist
    person_high_id: str | None = None
    # Feature engineering
    features: FriendshipFeatures | None = None
